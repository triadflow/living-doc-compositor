...
const docsScriptsProfilesDir = path.join(docsScriptsDir, 'living-doc-profiles');
...
export async function syncCompositorEmbeds(options = {}) {
  ...
  // Publish the source files the build-kit gallery + builder pages need to
  // fetch at runtime. GH Pages serves docs/ as root, so anything those pages
  // load must live inside docs/. We mirror the relevant scripts/ entries into
  // docs/scripts/ on every sync.
  const buildAssetSources = [
  ...
  const profileFilenames = (await readdir(profilesDir)).filter((n) => n.endsWith('.json')).sort();
  for (const filename of profileFilenames) {
    buildAssetSources.push([
      path.join(profilesDir, filename),
      path.join(docsScriptsProfilesDir, filename),
    ]);
  }

  let buildAssetsChanged = false;
  if (write) {
    await mkdir(docsScriptsDir, { recursive: true });
    await mkdir(docsScriptsProfilesDir, { recursive: true });
  }
  for (const [src, dst] of buildAssetSources) {
    const srcContent = await readFile(src, 'utf8');
    const existing = await readFile(dst, 'utf8').catch(() => null);
    if (existing !== srcContent) {
      buildAssetsChanged = true;
      if (write) {
  ...
  return {
    changed: compositorChanged || overviewChanged || buildAssetsChanged,
    compositor: { path: compositorPath, changed: compositorChanged },
    overview: { path: overviewPath, changed: overviewChanged },
    buildAssets: { path: docsScriptsDir, changed: buildAssetsChanged, count: buildAssetSources.length },
  };
}
...
if (isDirectRun) {
  ...
  const targets = [
    { ...result.compositor, label: path.relative(repoRoot, result.compositor.path) },
    { ...result.overview, label: path.relative(repoRoot, result.overview.path) },
    { ...result.buildAssets, label: `${path.relative(repoRoot, result.buildAssets.path)}/ (${result.buildAssets.count} files)` },
  ];
  if (checkOnly) {
    const drifted = targets.filter((t) => t.changed);
    if (drifted.length > 0) {
      for (const t of drifted) {
        console.error(`Out of sync: ${t.label}`);
      }
      process.exit(1);
    }
    for (const t of targets) {
      console.log(`Up to date: ${t.label}`);
    }
  } else {
    for (const t of targets) {
      console.log(`${t.changed ? 'Synced' : 'Already up to date'} ${t.label}`);
    }
  }
}
