...
The primary deliverable is a beautiful standalone static HTML page, not a chat-only report.
...
2. Accountability readout
   - large direct completion posture
   - explicit "implementation alone: yes/no" line
   - explicit scheduling-basis line

3. Gate dashboard
   - compact counts by state: closed, partial, open, blocked, unclear
   - compact counts by accountability type
   - count of gates with owner required

4. Closure path
   - one visually distinct gate block per gate
   - state badge
...
7. Scope and finish-language check
   - map vague finish labels to explicit closure criteria
   - state which finish claims become invalid if gates are skipped

8. Manager-facing summary
   - short summary that compresses accountability without comfort language
```

Visual requirements:

- Use a serious operational dashboard style: restrained, dense, readable, and print-friendly.
- Use a light background with high-contrast text.
...
- Suggested status colors:
  ...
  - blocked: red
  - unclear: gray
- Use a wide desktop layout with a sticky left summary rail when there is enough space.
- Collapse to a single-column mobile layout.
- Keep cards at 8px radius or less.
- Do not use decorative gradients, blobs, oversized hero treatment, or marketing copy.
- Do not hide required proof or owner-required labels behind hover-only UI.
- Make the page printable: avoid dark backgrounds, preserve section breaks, and include source path and timestamp.
...
After creating the page, report the generated HTML path and the source living doc path.
